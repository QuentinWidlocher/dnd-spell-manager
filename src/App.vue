<template>
	<div class="grid">
		<main>
			<router-view></router-view>
		</main>
		<footer class="h-16 z-20">
			<div class="navbar bg-wood-600 text-wood-900 flex align-middle justify-center h-full">
				<router-link to="/" class="button">
					<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path transform="rotate(90 12 12)" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
					</svg>
				</router-link>
				<router-link to="/settings" class="button">
					<svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
  						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
					</svg>	
				</router-link>
			</div>
		</footer>
	</div>
	<ReloadPrompt/>
</template>

<script setup lang="ts">
import './style.css'
import ReloadPrompt from './components/ReloadPrompt.vue';

</script>

<style>
.grid {
	width: 100%;
	height: 100%;

	overflow-y: hidden;

	display: grid;
	grid-template-columns: 1fr;
	grid-template-rows: 1fr auto;
}

main {
	width: 100%;
	height: 100%;
	min-height: 100%;

	overflow-y: auto;
	grid-row: 1;
}

footer {
	grid-row: 2;
}

.navbar {
	box-shadow: inset 0px 3px 1px rgb(141, 121, 103);
}

.button {
	@apply p-5;
}

.router-link-active {
	@apply text-white bg-wood-700;
	box-shadow: 
		inset 0px 4px 5px rgb(80, 71, 64),
		inset 0px -4px 15px rgb(141, 121, 103, .5);
}
</style>
